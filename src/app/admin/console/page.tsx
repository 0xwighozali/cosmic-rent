...
// Menggunakan tipe data yang lebih spesifik untuk status dan kondisi
type ConsoleStatus = "Available" | "In Use" | "Maintenance" | "Offline";
...
const Console: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"All" | ConsoleType>("All");
  ...
  // Menggunakan useMemo untuk menghitung statistik hanya saat data berubah
  const { ps4Available, ps5Available, ps4Maintenance, ps5Maintenance } =
    useMemo(
      () => ({
        ps4Available: consolesData.filter(
          (console) => console.type === "PS4" && console.status === "Available"
        ).length,
        ps5Available: consolesData.filter(
          (console) => console.type === "PS5" && console.status === "Available"
        ).length,
        ps4Maintenance: consolesData.filter(
          (console) =>
            console.type === "PS4" &&
            (console.status === "Maintenance" || console.status === "Offline")
        ).length,
        ps5Maintenance: consolesData.filter(
          (console) =>
            console.type === "PS5" &&
            (console.status === "Maintenance" || console.status === "Offline")
        ).length,
  ...
  // Fungsi utilitas untuk logika warna dan ikon
  ...
  const getStatusIcon = (status: ConsoleStatus) => {
    switch (status) {
      case "Available":
  ...
  const filteredConsoles = consolesData.filter((console) => {
    const matchesTab = activeTab === "All" || console.type === activeTab;
    const matchesStatus =
      ...
      <div className="grid grid-cols-1 gap-6 mb-8">
        <div>
          ...
          >
            ...
            <div className="w-full overflow-x-auto rounded-xl border border-gray-200/50 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
