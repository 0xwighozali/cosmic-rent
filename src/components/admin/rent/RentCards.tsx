...
import { User, Clock, Coffee, Eye, Printer, Play, Pause } from "lucide-react";
...
const RentCards: React.FC<RentCardsProps> = ({ unit, index }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className="bg-white/80 backdrop-blur-xl border border-gray-200 rounded-2xl p-6 
        hover:shadow-lg hover:shadow-gray-200/30 transition-all duration-300 flex flex-col"
    >
      {/* Unit Header */}
      ...
      {/* Unit Number */}
      <h3 className="text-xl font-bold text-gray-900 mb-3">{unit.number}</h3>
      ...
      <div className="mt-auto">
        {unit.status === "In Use" ? (
          ...
          <button
            className="w-full flex items-center justify-center gap-1 py-3 px-4 
            bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 
            text-white rounded-xl transition-all duration-200 text-sm font-medium shadow-sm"
          >
            <Play size={14} />
          ...
          <button
            disabled
            className="w-full flex items-center justify-center gap-1 py-3 px-4 
              bg-gray-100 text-gray-400 rounded-xl text-sm font-medium cursor-not-allowed"
          >
            <Pause size={14} />
